...
impl LaunchConfig {
    #[must_use]
    pub fn dataplane(&self, worker_index: usize) -> yayatht_dataplane::reactor::Config {
        debug_assert!(worker_index < self.workers);
        yayatht_dataplane::reactor::Config {
            ...
            max_tcp_flows: worker_share(self.max_tcp_flows, self.workers, worker_index),
            udp_enabled: self.udp_enabled,
            max_udp_flows: worker_share(self.max_udp_flows, self.workers, worker_index),
            max_udp_associations: worker_share(
                self.max_udp_associations,
                self.workers,
                worker_index,
            ),
            worker_count: self.workers,
            max_pending_tcp_bytes: worker_share(
                self.max_pending_tcp_bytes,
                self.workers,
                worker_index,
            ),
            max_retained_tcp_bytes: worker_share(
                self.max_retained_tcp_bytes,
                self.workers,
                worker_index,
            ),
...
pub enum ConfigError {
    ...
    InvalidProxyUdp,
    #[error("workers must be between 1 and 256")]
    InvalidWorkerCount,
    #[error("flow limits must provide at least one entry per worker")]
    InsufficientWorkerFlows,
    #[error("TCP byte limits must provide at least 16 KiB per worker")]
    InsufficientWorkerBytes,
}
...
impl LaunchConfig {
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        .into_iter()
        .any(|limit| limit < self.workers)
        {
            return Err(ConfigError::InsufficientWorkerFlows);
        }
        ...
        let worker_byte_floor = self.workers.saturating_mul(16 * 1024);
        if self.max_pending_tcp_bytes < worker_byte_floor
            || self.max_retained_tcp_bytes < worker_byte_floor
        {
            return Err(ConfigError::InsufficientWorkerBytes);
        }
...
const fn worker_share(total: usize, workers: usize, worker_index: usize) -> usize {
    total / workers + if worker_index < total % workers { 1 } else { 0 }
}

#[cfg(test)]
mod tests {
    use super::{first_nameserver, worker_share};
    ...
    #[test]
    fn worker_shares_preserve_the_configured_total() {
        let shares = (0..4)
            .map(|worker| worker_share(10, 4, worker))
            .collect::<Vec<_>>();
        assert_eq!(shares, [3, 3, 2, 2]);
        assert_eq!(shares.into_iter().sum::<usize>(), 10);
    }
}
